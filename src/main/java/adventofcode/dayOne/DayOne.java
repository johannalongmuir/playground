...
public class DayOne implements Runner {
    ...
    protected Integer calculateTotalFuelRequirement(String mass) {
        ...
        for (String s : split) {
            Integer initialFuelAmount = FuelCalculator.fuelByMassCalculator(Integer.parseInt(s));
            fuelCounterUpper += initialFuelAmount;

            if (FeatureHandler.FEATURE_MASS_OF_FUEL_ADDED_TO_TOTAL) {

                while (initialFuelAmount > 0) {
                    initialFuelAmount = FuelCalculator.fuelByMassCalculator(initialFuelAmount);
...
//                        Integer fuel = FuelCalculator.fuelByMassCalculator(additionalFuelCalculation);
//                        if (fuel >= 0) {
